...
class Slider:
    def __init__(self, pos: tuple, size: tuple, label: str, initial_val: float, min: int, max: int) -> None:
        ...
        self.button_rect = pygame.Rect(self.slider_left_pos + self.initial_val - 5, self.slider_top_pos + 9, 10, self.size[1])
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        slider_rect = pygame.Rect(screen_width/20, self.pos[1]-screen_width/52, screen_width/1.1, screen_height//15)
        pygame.draw.rect(screen, WHITE, slider_rect)
        
        text_num_surface = self.render
        text_num_rect = text_num_surface.get_rect(center = (screen_width - screen_width/13, self.pos[1]+screen_height//100))
